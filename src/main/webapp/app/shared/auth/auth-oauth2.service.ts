...
export class AuthServerProvider {
    ...
    login (credentials): Observable<any> {
        let data = 'username=' +  encodeURIComponent(credentials.username) + '&password=' +
            ...
            'client_secret=my-secret-token-to-change-in-production&client_id=SpeakTogetherapp';
        let headers = new Headers ({
            'Content-Type': 'application/x-www-form-urlencoded',
            'Accept': 'application/json',
            'Authorization': 'Basic ' + this.base64.encode('SpeakTogetherapp' + ':' + 'my-secret-token-to-change-in-production')
        });
